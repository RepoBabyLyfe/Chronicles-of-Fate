...
public record GameState(
    ...
    public static GameState withCombat(int playerHp, int playerFocus, int enemyHp,
                                       int etherFragments, Set<String> unlockedCards, int defeatedBosses) {
        return new GameState(playerHp, playerFocus, enemyHp, etherFragments, unlockedCards, true, defeatedBosses);
    }

    //costruttore per salvataggi solo-profilo (dal negozio o menu)
    public static GameState profileOnly(int etherFragments, Set<String> unlockedCards, int defeatedBosses) {
        return new GameState(0, 0, 0, etherFragments, unlockedCards, false, defeatedBosses);
    }
}